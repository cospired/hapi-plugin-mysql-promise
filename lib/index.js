...
internals.getConnection = (_callback) => internals.pool.getConnection();
...
exports.getConnection = internals.getConnection;

internals.stop = (server, next) => {

  if (typeof server === 'function' && !next) {
    // eslint-disable-next-line no-param-reassign -- common practice for last element
    next = server;
  }
...
internals.init = (baseOptions, callback) => {
  ...
  if (!internals.pool) {
    callback(new Error('No mysql pool found'));
...
function register(server, baseOptions) {
